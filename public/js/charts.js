...
import { cssVar, moneyShort, money, esc } from './util.js';
...
export function tooltipBase() {
  ...
  return {
    ...
    extraCssText: 'border-radius:10px;box-shadow:0 16px 40px -16px rgba(0,0,0,.55);',
    confine: true,
  };
}
...
window.addEventListener('resize', () => { for (const c of instances) c.resize(); });

// Tooltip row helper: colored key + label + value, text stays in ink colors
export const ttRow = (color, label, value, bold = false) =>
  `<div style="display:flex;align-items:center;gap:8px;min-width:170px;margin-top:3px">
...
export const ttHead = (t) => `<div style="font-weight:650;margin-bottom:4px">${esc(t)}</div>`;
