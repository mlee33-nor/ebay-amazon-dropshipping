...
export async function renderListingsPanel(box) {
  ...
  box.innerHTML = `${head}${trafficNote}
    <div class="kpis five">
      ...
      ${kpi('To refresh or remove', `<span class="${L.stale.count ? 'neg' : ''}">${count(L.stale.count)}</span>`, L.stale.criteria.viewsRule === 'no views at all' ? '30+ days live, no sales, no views' : 'live 30+ days, no sales', `Live 30+ days, nothing sold${L.stale.criteria.viewsRule ? `, and ${L.stale.criteria.viewsRule} in the last 30 days` : ''}`)}
    </div>
